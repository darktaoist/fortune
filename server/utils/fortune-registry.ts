...
const LIFE_SECTION_GUIDE = [
  ...
  '각 대운은 한 줄 라벨로 시작한다 — "회차 · 간지 · 나이 구간 · 연도 구간"을 가운뎃점(·)으로 구분한다.',
  '  · 라벨과 본문의 모든 표현(회차·나이·연도 단위어 등)은 반드시 출력 언어(아래 "모든 본문은 …로 작성한다"에 지정된 언어)로 쓴다. 현재 대운이면 라벨 끝에 현재 대운임을 출력 언어(아래 "모든 본문은 …로 작성한다"에 지정된 언어)로 표시한다.',
  '  · 간지는 한자 그대로 둔다(예: 癸未). 독음을 병기하려면 출력 언어(아래 "모든 본문은 …로 작성한다"에 지정된 언어) 기준으로만 한다 — 한국어가 아닌 언어에서 한글 독음(예: "계미")을 쓰지 않는다.',
  '이어서 그 대운의 오행 기운, 원국과의 상호작용, 구체적 변화 시기를 2~4문장으로(역시 출력 언어(아래 "모든 본문은 …로 작성한다"에 지정된 언어)로) 풀이한다. 대운 사이에는 빈 줄을 둔다.',
  '',
